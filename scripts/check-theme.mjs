...
for (const blocked of [false, true]) for (const reduced of [false, true]) for (const transitions of [false, true]) for (const entry of ['top', 'fragment', 'scroll-x', 'scroll-y', 'back_forward']) {
	...
	const context = vm.createContext({
		...
		location: { hash: entry === 'fragment' ? '#section' : '' },
		scrollX: entry === 'scroll-x' ? 100 : 0, scrollY: entry === 'scroll-y' ? 800 : 0,
		performance: { getEntriesByType: () => [{ type: entry === 'back_forward' ? entry : 'navigate' }] },
	...
	assert.equal(classes.has('px-wait'), !reduced && entry === 'top', 'Only a fresh top entry with normal motion hides text for the entrance.');
	clicked();
	assert.equal(root.dataset.theme, 'light');
	assert.equal(label, 'Switch to dark mode');
	assert.equal(events, 1);
	assert.equal(transitionCount, transitions && !reduced ? 1 : 0, 'Reduced motion changes theme immediately without a view transition.');
